...
if [[ "${1:-}" == "repo" && "${2:-}" == "view" ]]; then
  echo '{"nameWithOwner":"PanicScan/panic-scan-open","url":"https://github.com/PanicScan/panic-scan-open","defaultBranchRef":{"name":"main"}}'
  exit 0
fi

if [[ "${1:-}" == "api" ]]; then
  case "${2:-}" in
    repos/PanicScan/panic-scan-open/actions/workflows/ci.yml|repos/PanicScan/panic-scan-open/actions/workflows/release.yml)
      echo '{}'
      exit 0
      ;;
  esac
...
if PATH="$fake_bin:$PATH" scripts/repository_readiness_audit.sh PanicScan/panic-scan-open >"$blocked_output" 2>&1; then
  echo "expected repository readiness audit to fail when signing configuration is missing" >&2
  exit 1
fi

for expected in \
  "PASS GitHub repository is visible to gh: PanicScan/panic-scan-open" \
...
PANICSCAN_FAKE_READINESS_READY=1 \
  PATH="$fake_bin:$PATH" \
  scripts/repository_readiness_audit.sh PanicScan/panic-scan-open >"$ready_output" 2>&1
