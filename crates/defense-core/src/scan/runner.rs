        ...
        if deadline.is_some_and(|d| Instant::now() >= d) {
            ...
            for socket in sockets {
                ...
                findings.push(Finding {
                    ...
                    explanation: "A process is communicating with a known malicious IP or using a suspicious port.".to_string(),
...
///   2. `PANICSCAN_SCAN_MAX_MINUTES` env var (used by CI smoke tests)
...
fn effective_deadline(request: &ScanRequest) -> Option<Instant> {
    ...
    if let Ok(val) = std::env::var("PANICSCAN_SCAN_MAX_MINUTES") {
