...
pub fn analyze_network_connection(item: &NetworkItem) -> Vec<Evidence> {
    ...
    // Kara Liste IP Adresleri (P2P Threat Intelligence'dan okunur)
    if let Some(remote_ip) = &item.remote_ip {
        if ThreatDatabase::global().is_bad_ip(remote_ip.as_str()) {
            evidences.push(Evidence {
                kind: crate::report::EvidenceKind::Network,
                code: "network.blacklist_ip".to_string(),
                title: "Malicious IP Connection".to_string(),
                detail: format!("Connection to known malicious C2 IP: {}", remote_ip),
                weight: 100,
            });
        }
    }
