...
fn analyze_file_bytes_for_path(
    ...
    // --- Makine Öğrenimi (ONNX) Çevrimdışı Sınıflandırma ---
    let entropy = calculate_entropy(bytes);
    let size = bytes.len() as f32;
    let is_packed = if entropy > 7.2 { 1.0 } else { 0.0 }; // Simple static heuristic
    ...
    if let Ok(ml_score) = panicscan_ml::MlEngine::global().predict([
        ...
        if ml_score >= 0.80 {
            evidences.push(crate::report::Evidence {
                kind: crate::report::EvidenceKind::File,
                code: "ml.malicious".to_string(),
                title: "Machine Learning Detection".to_string(),
                detail: format!("High probability of malware (Score: {:.2})", ml_score),
                weight: 85,
            });
        }
    }
